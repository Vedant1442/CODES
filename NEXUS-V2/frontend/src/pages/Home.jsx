    ...
    <div className="w-full">
      ...
      <div className="mb-10">
        ...
        {categories.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3">
            {categories.map((cat, i) => (
              <motion.button
                ...
                className="flex flex-col items-center gap-3 p-4 rounded-2xl border border-gray-100 bg-white shadow-sm cursor-pointer transition hover:border-nexus-green group"
              >
                <div className="w-16 h-16 rounded-xl bg-gray-50 flex items-center justify-center overflow-hidden">
                  <img src={cat.image} alt={cat.name} className="w-full h-full object-contain p-2 group-hover:scale-110 transition-transform" />
                </div>
          ...
          <div className="flex items-center gap-2 py-4 text-gray-400 font-medium">
            <Loader2 className="w-4 h-4 animate-spin" />
            Loading Blinkit Categories...
          </div>
        )}
      </div>
      ...
      {featuredProducts.length > 0 && (
        <div className="mb-12">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-black text-gray-900 tracking-tight">Featured on Blinkit</h2>
            <button
              onClick={() => navigate('/search?q=milk')}
              className="text-primary font-bold text-sm flex items-center gap-1 hover:underline"
            >
      ...
      {/* Fallback Static Rails if no dynamic content */}
      {featuredProducts.length === 0 && (
        <>
