  ...
  if (!isConnected) {
    return (
      <div className="flex flex-col items-center justify-center pt-32 gap-4 text-center px-4">
        ...
        <h2 className="text-xl font-black text-gray-800">Connecting to Warp Pool...</h2>
      </div>
    );
  }
  ...
  if (!isLocationSet) {
    return (
      <div className="flex flex-col items-center justify-center pt-32 gap-4 text-center px-4">
        ...
        <h2 className="text-2xl font-black text-gray-800">Set your location</h2>
        <p className="text-gray-500 text-sm">Click the picker above to see Blinkit prices.</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full pb-16">
      <div className="mb-8">
        ...
        <p className="text-gray-400 text-sm font-medium mt-1">{isSearching ? statusMessage : `${(products.blinkit || []).length} items found`}</p>
      </div>
        ...
        <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          {!isSearching && !hasAnyResults && (
            <div className="flex flex-col items-center justify-center pt-16 gap-4 text-center">
              ...
              <p className="text-gray-400 text-sm">Try searching for something else like "milk" or "chips".</p>
            </div>
          )}
        </motion.div>
      </AnimatePresence>
